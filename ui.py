...
class AFFINITYBRIDGE_PT_Panel(bpy.types.Panel):
    ...
    def affinity_bridge(self, af_bool):
        ...
        if af_bool == False:
            col = layout.column(align=True)
            col.prop(prefs, 'is_display_filepath', text="Output Software Settings") 
            col.label(icon='ERROR', text='Note: File path is displayed when enabled')  
            if is_display_filepath == True:
                col.prop(prefs, 'option_image_editing_exe', text="Output Software") 
    ...
    def used_original_name(self):
        ...
        col = layout.column(align=True)
        col.prop(scene.affinitybridge, 'is_change_name', text='Use Original Name')
    ...
    def draw(self, context):
        ...
        try:
            # Determine if the file path does not exist
            if context.space_data.image.filepath_from_user(image_user=None) == '':
                # File path does not exist
                self.render_setting()
                self.used_original_name()
                # Choose whether to integrate with Affinity Photo V2 or link with external software
                # Affinity Bridge button
            ...
            col = layout.column(align=True)
            col.label(text='Please open a valid image', icon='ERROR')
...
class AFFINITYBRIDGE_PT_InformationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        layout = self.layout
        box = layout.box()
        box.label(icon='FILE_FOLDER', text='Save Location')
